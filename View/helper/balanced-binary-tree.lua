...
function AVL:insert(v)
  v.index = math.min(v.p1.x, v.p2.x)
  ...
  if v == self.value then
    return self,0
  elseif v.index < self.value.index then
...
function AVL:delete(v,isSubtree)
  v.index = math.min(v.p1.x, v.p2.x)
  local grow=0
  if v.index == self.value.index then
    print("delegint")
    local v
    ...
    self.value=v
    self.balance=self.balance+grow
  elseif v.index <self.value.index and self.left then
    self.left,grow=self.left:delete(v,true)
    self.balance=self.balance-grow
  elseif v.index >self.value.index and self.right then
...
function AVL:dump(depth)
  ...
  print(string.rep("    ",depth)..self.value.index.." ("..self.balance..")")
